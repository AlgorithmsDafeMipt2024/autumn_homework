...
double DeikstraAlgorithm(const WeightedGraph& graph, int start_id, int end_id)
{
	...
	for (; true; )
	{
		...
		{
			if (graph[this_id][i])
				pathLength[i] = std::min(pathLength[i], pathLength[this_id] + graph[this_id][i]);
		}
			ended[this_id] = true;
			double min = DBL_MAX;
		for (int j = 0; j < n; ++j)
...
double DeikstraAlgorithm(const OrientedWeightedGraphWithZeroWeight& graph, int start_id, int end_id)
{
	...
	for (; true; )
	{
		...
		{
			if (graph[this_id][i])
				pathLength[i] = std::min(pathLength[i], pathLength[this_id] + graph.GetWeight(this_id, i));
		}
		ended[this_id] = true;
		double min = DBL_MAX;
		for (int j = 0; j < n; ++j)
...
DeikstraReturn DeikstraPathAlgorithm(const WeightedGraph& graph, int start_id, int end_id)
{
	...
	for (int counter = 0; counter < n && this_id != end_id; ++counter)
	{
		for (int i = 0; i < n; ++i)
		{
			if (graph[this_id][i] && pathLength[i] > pathLength[this_id] + graph[this_id][i])
			{
	...
	std::vector<int> path;
	for (int i = end_id; i != start_id ; i = prevs[i])
		path.push_back(prevs[i]);
...
{
	if (start_id == end_id)
		return DeikstraReturn(std::vector<int>(), 0);
	int n = graph.GetVerts();
	std::vector<bool> ended(n);
	std::vector<double> pathLength(n, DBL_MAX);
	std::vector<int> prevs(n, -1);
	...
	for (int counter = 0; counter < n && this_id != end_id; ++counter)
	{
		for (int i = 0; i < n; ++i)
		{
			if (graph[this_id][i] && pathLength[i] > pathLength[this_id] + graph.GetWeight(this_id, i))
			{
