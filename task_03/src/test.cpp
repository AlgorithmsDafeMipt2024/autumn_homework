...
bool operator==(const OrientedWeightedGraphWithZeroWeight& first, const OrientedWeightedGraphWithZeroWeight& second)
{
	if (first.GetVerts() != second.GetVerts())
		return false;
	for (int i = 0; i < first.GetVerts(); ++i)
		for (int j = 0; j < first.GetVerts(); ++j)
			if (first[i][j] != second[i][j] || first.GetWeight(i, j) != second.GetWeight(i, j))
				return false;
	return true;
}
