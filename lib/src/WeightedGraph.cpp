...
bool WeightedGraph::IsCycled(int id, std::vector<int>& result,
                             std::vector<bool>& visited) const {
  for (int i = 0; i < graph.size(); ++i) {
    if (graph[id][i] && !visited[i]) {
      visited[i] = true;
      if (IsCycled(i, result, visited)) return true;
      result.push_back(i);
    } else if (graph[id][i] && visited[i] && i != id) {
      if (!in(result, i)) return true;
    }
  }
  return false;
}
...
bool WeightedGraph::SwapIds(int id0, int id1) {
  if (std::max(id0, id1) >= graph.size() || id0 == id1) return true;
...
void WeightedGraph::DFS(int id, std::vector<int>& result,
                        std::vector<bool>& visited, bool before) {
  for (int i = 0; i < graph.size(); ++i) {
    if (graph[id][i] && !visited[i]) {
      visited[i] = true;
      if (!before) result.push_back(i);
...
std::vector<int> WeightedGraph::DFS(bool before) { return DFS(0, before); }

void WeightedGraph::BFS(int id, std::vector<int>& result,
  ...
  for (int i = 0; i < need_visit.size(); ++i) {
    if (!visited[need_visit[i]]) BFS(need_visit[i], result, visited);
  }
}
...
std::vector<int> WeightedGraph::BFS() { return BFS(0); }
