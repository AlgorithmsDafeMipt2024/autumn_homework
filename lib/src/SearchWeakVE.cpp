...
void dfs(WeakVE& result, const WeightedGraph& graph, int id, int parent,
         std::vector<int>& disc, std::vector<int>& low,
         std::vector<int>& visited, int& time) {
  disc[id] = time;
  low[id] = time;
  ++time;
  visited[id] = true;
  for (int i = 0; i < graph.GetVerts(); ++i) {
    if (graph[id][i] && !visited[i]) {
      ...
      if (low[i] > disc[id]) result.second.push_back(std::make_pair(id, i));
      if (parent != -1 && low[i] >= disc[id]) result.first.push_back(id);
...
WeakVE SearchWeakVE(const WeightedGraph& graph) {
  ...
  for (int i = 0; i < graph.GetVerts(); ++i) {
    if (graph[start_id][i] != 0) ++num_start_id_daughters;
  }
  if (num_start_id_daughters > 1) result.first.push_back(start_id);
  return result;
}
