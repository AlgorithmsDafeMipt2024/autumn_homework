...
void weak_vertex(std::vector<std::vector<int>> &data,
                 std::vector<bool> &visited, std::vector<int> &time_in,
                 std::vector<int> &min_time_in, std::vector<int> &cut_verts,
                 int time, int v, int p = -1) {
  visited[v] = true;
  ...
  for (int i = 0; i < data[v].size(); ++i) {
    ...
    if (visited[data[v][i]]) {
      ...
      weak_vertex(data, visited, time_in, min_time_in, cut_verts, time,
                  data[v][i], v);
      count++;
