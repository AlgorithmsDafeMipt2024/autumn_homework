...
inline void FindBridgesStepDFS(
...
inline std::vector<std::pair<vert_t, vert_t>> FindBridges(
  ...
  for (const auto& vert : graph.Verts())
    if (!visited[vert])
      FindBridgesStepDFS(vert, vert, curr_index, indexes, low_links, visited,
                         adj_list, bridges);
