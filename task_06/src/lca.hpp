...
class LCA {
 public:
  ...
  vert_t RequestLCA(vert_t left, vert_t right) {
    ...
    ssize_t l = first_pos_in_euler_tour_[left],
            r = first_pos_in_euler_tour_[right];
    ...
    if (block_l == block_r)
      return euler_tour_[LCAInBlock_(block_l, l % block_, r % block_)];
 ...
 private:
  ...
  ssize_t MinHeight_(ssize_t i, ssize_t j) {
    return vert_heights_[euler_tour_[i]] < vert_heights_[euler_tour_[j]] ? i
                                                                         : j;
  }
...
vert_t CalculateLCA(const Graph<vert_t, weight_t>& graph, vert_t root,
  ...
  return LCA<vert_t, weight_t>(graph, root).RequestLCA(left, right);
}